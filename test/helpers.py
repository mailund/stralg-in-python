...
def collect_tests(tests: Iterable[tuple[str, Fn[..., None]]]) -> type:
...
def pick_random_patterns_len(x: str, n: int, patlen: int) -> Iterator[str]:
    ...
    for _ in range(n):
        i = random.randrange(1, len(x) - 1)  # -1 because we don't want ""
...
def check_matches(x: str, p: str, matches: Iterable[int]) -> None:
    ...
    for i in matches:
        assert check_substring(x, p, i), f"Substring {x[i:]} should match pattern {p}"


def check_equal_matches(x: str, p: str, *algos: Fn[[str, str], Iterator[int]]) -> None:
